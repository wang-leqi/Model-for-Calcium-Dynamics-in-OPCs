...
def SPDE_solver(ICs = [0.16, 30.0, 0.64, 0.88, 0.61, 0.12, 13.0, 0.0],  # Initial conditions
    ...
    def Jleak_Ca(volt):
        alpha=1/(2*Vosteo*F)
        return -fleak*alpha*Ileak_CaNa(volt)*10**6
    
    def Jleak_Na(volt):
        alpha=1/(Vosteo*F)
        return -(1-fleak)*alpha*Ileak_CaNa(volt)*10**3
    ...
    def dK(Ca,K,Na,volt,O,On,Q1,Q2,Q3,Q4,Kout):
        return 2*Jnak(Ca, Na, K,volt,Kout)-Jp2x7_K(Ca, Na, K, Q1, Q2, Q3, Q4,volt)-Jnmda_K(On, Ca, Na, K,volt)-Jampa_K(O, Ca, Na, K,volt)+Jk(K, volt,Kout)+Jrk(K)
    
    def fvolt(Ca,Cer,K,Na,volt,O,On,Q1,Q2,Q3,Q4,mT,hTf,mL,hL,Kout):
        return (-Iampa(O, Ca, Na, K,volt)-Inmda(On, Ca, Na, K,volt)-Ip2x7(Ca, Na, K, Q1, Q2, Q3, Q4,volt)-Isoc(Ca,Cer,volt)-Ik(K,volt,Kout)-Inak(Ca,Na,K,volt,Kout)-Incx(Ca, Na, volt)-Il(Ca,mL,hL,volt)-It(Ca,mT,hTf,volt)-Ileak_CaNa(volt)-Ileak(K,volt,Kout))/(Cm*sur)*100
    ...
    for k in range(n-1):
        ...
        if rdmGlut==True :
            #print("rdm Glut")
            if np.random.binomial(1, 0.005)==1:# and k>5000:
                        trace=k
                        rdn=np.abs(np.random.normal(0,0.5e-3)) #1e-3
            if k<trace+10:
                        Glut=rdn
        #saveA[k]=Glut
...
# ============================================================
# np.random.seed(2) 
# temp = SPDE_solver(ICs, rdmGlut=True, tmax = 300)
# volt = temp[1].T[4]
# Jsoc = temp[8]
# Jip3 =temp[10]
# Jryr = temp[11]
